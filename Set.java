 ...
 * @throws InvalidNodeException 
  ...
  public void union(Set s) {
	  if (s == null) 
		  ...
		  try {
			  ListNode current = s.myList.front();
			  while (current != null) {
				  this.insert((Comparable) s.myList.front());
//				  size++;
				  current = current.next();
			  }
  ...
  public void intersect(Set s) {
	  if (s == null) 
		  ...
		  try {
			  ListNode current = this.myList.front();
			  while (current != null) {
				  if (!ifInList(current,s.myList)) {
					  current.remove();
				  }
				  current = current.next();
			  }			  
  ...
  private boolean ifInList(ListNode node, List s) {
